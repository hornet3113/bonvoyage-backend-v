...
export async function POST(_req: Request, { params }: Params) {
  ...
  try {
    ...
    const result = await db.query<{ fn_change_trip_status: string }>(
      `SELECT fn_change_trip_status($1, $2, 'COMPLETE') AS new_status`,
      [tripId, userId]
    )
    ...
    console.error('[POST /trips/:tripId/complete]', error)
    if (error instanceof Error) {
      if (error.message.includes('not found or access denied'))  return err('Trip not found', 404)
      if (error.message.includes('Only CONFIRMED'))              return err(error.message, 400)
      if (error.message.includes('end_date has not passed'))     return err(error.message, 400)
    }
