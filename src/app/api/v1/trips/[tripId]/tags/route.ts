...
export async function GET(
  ...
  try {
    ...
    const owns = await verifyTripOwnership(params.tripId, internalUserId)
...
export async function POST(
  ...
  try {
    ...
    const result = await db.query(
      ...
      [params.tripId, body.data.tag_id]
    )
    return ok(result.rows[0] ?? { trip_id: params.tripId, tag_id: body.data.tag_id }, 201)
  } catch (e: any) {
    return err(e.message, 500)
  }
}
